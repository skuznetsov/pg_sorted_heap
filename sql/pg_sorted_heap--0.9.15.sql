...
IS 'Hierarchical HNSW search via sidecar PG tables. prefix names the table family: {prefix}_meta, {prefix}_l0..lN, and optionally {prefix}_r1 (nid int4 PK, rerank_vec hsvec). Pipeline: greedy ef=1 descent through upper levels → ef_search beam at L0 (hsvec(384) sketch) → optional dense rerank via {prefix}_r1 keeping top rerank1_topk → exact svec cosine rerank via main table keeping top lim. rerank_topk controls how many L0 candidates reach exact rerank: 0 (default) means no truncation (all ef_search candidates are reranked); a value 0 < rerank_topk < ef_search truncates to rerank_topk before exact rerank, reducing TOAST I/O at the cost of recall. rerank_topk >= ef_search has no effect. Set rerank1_topk>0 to enable the dense r1 stage (requires _r1 sidecar populated with hsvec prefix embeddings); if the _r1 table is absent the stage is silently skipped. Enable sorted_heap.hnsw_cache_l0 for session-local L0+upper cache (~100 MB, built on first call). Timing breakdown in DEBUG1 when sorted_heap.ann_timing=on.';
