...
-- SH19-9: PARAM_EXEC array from LATERAL/NestLoop uses SortedHeapScan
CREATE FUNCTION sh19_make_arr(i int) RETURNS int[] AS $$
BEGIN
...
SELECT sh19_plan_contains(
    $$SELECT o.i, s.cnt
      FROM (SELECT g AS i, sh19_make_arr(g) AS arr FROM generate_series(1,2) g) o
      CROSS JOIN LATERAL (
        SELECT count(*) AS cnt FROM sh19 WHERE a = ANY(o.arr)
      ) s$$,
    'SortedHeapScan') AS sh19_lateral_plan;
-- SH19-10: LATERAL runtime ANY executes correctly (would crash before fix)
SELECT array_agg(s.cnt ORDER BY o.i)
FROM (SELECT g AS i, sh19_make_arr(g) AS arr FROM generate_series(1,2) g) o
CROSS JOIN LATERAL (
  SELECT count(*) AS cnt FROM sh19 WHERE a = ANY(o.arr)
) s;
\t off
DROP FUNCTION sh19_make_arr(int);
RESET enable_indexscan;
RESET enable_bitmapscan;
RESET enable_hashjoin;
RESET enable_mergejoin;
...
-- ================================================================
-- SH20: Two-pass scan (sorted prefix + conservative tail)
-- After compact, inserting beyond coverage creates an unsorted tail.
-- The two-pass scan should still use SortedHeapScan for prefix queries.
-- ================================================================

CREATE FUNCTION sh20_plan_contains(query text, pattern text) RETURNS boolean AS $$
DECLARE r record;
BEGIN
...
-- SH20-1: Create table, compact → fully sorted
CREATE TABLE sh20(id int PRIMARY KEY, val text) USING sorted_heap;
INSERT INTO sh20 SELECT g, repeat('x', 80) FROM generate_series(1, 5000) g;
SELECT sorted_heap_compact('sh20'::regclass);
ANALYZE sh20;

-- Verify: SortedHeapScan available on fully sorted table
SET enable_indexscan = off;
SET enable_bitmapscan = off;
SELECT sh20_plan_contains(
    'SELECT * FROM sh20 WHERE id = 100',
    'SortedHeapScan') AS sh20_sorted_ok;

-- SH20-2: Insert beyond coverage → unsorted tail created
DO $$
BEGIN
    FOR i IN 5001..6000 LOOP
        INSERT INTO sh20 VALUES (i, repeat('x', 80));
    END LOOP;
END;
$$;

-- Zone map should be valid but not sorted
SELECT CASE WHEN sorted_heap_zonemap_stats('sh20'::regclass)
                 LIKE '%flags=valid%'
              AND sorted_heap_zonemap_stats('sh20'::regclass)
                 NOT LIKE '%flags=valid,sorted%'
         THEN 'sh20_valid_not_sorted_ok'
         ELSE 'sh20_FAIL: ' || sorted_heap_zonemap_stats('sh20'::regclass)
    END AS sh20_2_zm_state;

-- SH20-3: Two-pass scan: prefix query should still use SortedHeapScan
-- (Previously this fell back to SeqScan because single-pass bounding box
-- included all blocks.)
SELECT sh20_plan_contains(
    'SELECT * FROM sh20 WHERE id = 100',
    'SortedHeapScan') AS sh20_two_pass_plan;

-- SH20-4: Correctness: prefix query returns correct result
SELECT id FROM sh20 WHERE id = 100;

-- SH20-5: Tail query also returns correct result
SELECT id FROM sh20 WHERE id = 5500;

-- SH20-6: Range query spanning prefix and tail
...
-- SH21B-3: Generic prepared composite bounds keep the same tight pruning.
SET plan_cache_mode = force_generic_plan;
PREPARE sh21b_runtime(int, int, int) AS
    SELECT * FROM sh21b WHERE tenant_id = $1 AND id BETWEEN $2 AND $3;
PREPARE sh21b_runtime_count(int, int, int) AS
...
-- SH21B-4: First-column-only range remains pruned and correct.
SELECT sh21b_zonemap_scanned(
...
-- SH21B-5: Unsorted tail stays conservative for second-column predicates.
...
-- ANN-8: sketch dimension exceeding query dimension raises ERROR
DO $$
BEGIN
	-- Create sidecar with 16-dim sketch vs 8-dim query → must error
	CREATE TEMP TABLE ann_sketch_bad (
		partition_id int2 NOT NULL,
		id text NOT NULL,
		sketch hsvec(16) NOT NULL,
		PRIMARY KEY (partition_id, id)
	);
	INSERT INTO ann_sketch_bad
	SELECT partition_id, id,
	       ('[' || array_to_string(ARRAY(
	           SELECT (random()*2-1)::float4 FROM generate_series(1,16)
	       ), ',') || ']')::hsvec
	FROM ann_test;

	PERFORM * FROM svec_ann_scan(
		'ann_test', '[5,3,6,7,1,2,4,2]'::svec,
		2, 5, 100,
		current_setting('ann_test.pq_cb_id')::int4,
		current_setting('ann_test.ivf_cb_id')::int4,
		'pq_code',
	...
	RAISE NOTICE 'ANN-8 FAIL: expected dimension error not raised';
EXCEPTION WHEN data_exception THEN
	RAISE NOTICE 'ANN-8 OK: %', SQLERRM;
END;
$$;

-- ANN-9: incomplete sidecar (missing rows) raises ERROR
DO $$
BEGIN
	CREATE TEMP TABLE ann_sketch_partial (
		partition_id int2 NOT NULL,
		id text NOT NULL,
		sketch hsvec(8) NOT NULL,
		PRIMARY KEY (partition_id, id)
	);
	-- Insert only 1 row — most candidates will miss
	INSERT INTO ann_sketch_partial
	SELECT partition_id, id, embedding::hsvec
	FROM ann_test LIMIT 1;
	...
	PERFORM * FROM svec_ann_scan(
		...
		current_setting('ann_test.ivf_cb_id')::int4,
		'pq_code',
		'ann_sketch_partial', 3
	);
	RAISE NOTICE 'ANN-9 FAIL: expected missing-row error not raised';
EXCEPTION WHEN data_exception THEN
	RAISE NOTICE 'ANN-9 OK: missing sidecar row detected';
END;
$$;

DROP TABLE ann_test_sketch;

-- ANN-10: reversed PK order in sidecar (id, partition_id) must still work
CREATE TABLE ann_test_sketch_rev (
    id text,
    partition_id int2,
    sketch hsvec(8),
    PRIMARY KEY (id, partition_id)
);
INSERT INTO ann_test_sketch_rev
SELECT id, partition_id, embedding::hsvec
FROM ann_test;

SELECT count(*) AS ann10_count,
       bool_and(distance >= 0) AS ann10_nonneg
FROM svec_ann_scan(
	'ann_test',
	'[5,3,6,7,1,2,4,2]'::svec,
	2, 5, 100,
