...
# Reproducible segmented GraphRAG multidepth benchmark runner for a
# user-provided AWS host.
#
# It syncs the current repo subset, installs the extension remotely, and runs
# the segmented synthetic multidepth benchmark on the target host.
#
# Usage:
#   AWS_HOST=<user@host> AWS_REMOTE_DIR=/path/to/repo \
#     ./scripts/bench_graph_rag_multidepth_segmented_aws.sh [host] [remote_dir] [port]

HOST="${1:-${AWS_HOST:-}}"
REMOTE_DIR="${2:-${AWS_REMOTE_DIR:-}}"
PORT="${3:-${AWS_PORT:-65494}}"
...
echo "== aws segmented multidepth preflight =="
...
echo "== run remote segmented multidepth benchmark =="
REMOTE_CMD=$(
  cat <<EOF
set -euo pipefail
cd '$REMOTE_DIR'
'$REMOTE_PYTHON' scripts/bench_graph_rag_multidepth_segmented.py \
  --port '$PORT' \
  --tmp-root '$TMP_ROOT' \
  --num-pairs '$NUM_PAIRS' \
  --max-depth '$MAX_DEPTH' \
