...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            prompt_lexseed_require_summary_snippet_sql = base.QueryCase(
                ...
                require_seed AS MATERIALIZED (
                    ...
                    FROM {{table}}
                    WHERE entity_id = ANY (ARRAY(SELECT entity_id FROM lexical_seed))
                      AND relation_id = {REL_REQUIRES_FILE}
                ),
                ann_summaries AS MATERIALIZED (
                    SELECT
                        ...
                        0 AS source_rank
                    FROM {{table}} s
                    WHERE s.entity_id = ANY (ARRAY(SELECT entity_id FROM ann))
                      AND s.relation_id = {REL_FILE_SUMMARY}
            ...
            prompt_summary_chunk_semantic_s3_sql = base.QueryCase(
                "prompt_summary_chunk_semantic_s3_in",
                f"""
                WITH ann AS MATERIALIZED (
                    SELECT entity_id
                    FROM {{table}}
                ...
                best_chunks AS MATERIALIZED (
                    ...
                    FROM (
                        SELECT
                            ...
                            row_number() OVER (
                                PARTITION BY c.entity_id
                                ORDER BY c.embedding <=> %s::svec, c.target_id
                            ) AS chunk_rank
                        FROM {{table}} c
                        WHERE c.entity_id = ANY (
                            ARRAY(
            ...
            cases: list[tuple[str, str, base.QueryCase]] = [
                ...
                ("facts_heap", "facts_heap", prompt_summary_chunk_semantic_s3_sql),
                ("facts_sh", "facts_sh", prompt_summary_chunk_semantic_s3_sql),
