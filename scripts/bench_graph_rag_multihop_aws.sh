...
# Reproducible GraphRAG multihop benchmark runner for a user-provided AWS host.
#
# It syncs the current repo subset, installs the extension remotely, and runs
# the fact-shaped multihop benchmark on the target host. This mirrors the
# Gutenberg AWS wrappers, but targets bench_graph_rag_multihop.py directly.
#
# Usage:
#   AWS_HOST=<user@host> AWS_REMOTE_DIR=/path/to/repo \
#     ./scripts/bench_graph_rag_multihop_aws.sh [host] [remote_dir] [port]

HOST="${1:-${AWS_HOST:-}}"
REMOTE_DIR="${2:-${AWS_REMOTE_DIR:-}}"
PORT="${3:-${AWS_PORT:-65491}}"
...
echo "== run remote multihop benchmark =="
REMOTE_CMD=$(
  cat <<EOF
set -euo pipefail
cd '$REMOTE_DIR'
'$REMOTE_PYTHON' scripts/bench_graph_rag_multihop.py \
  --port '$PORT' \
  --num-pairs '$NUM_PAIRS' \
