...
Use [Stable API](docs/api-stable.md) for the compact application-facing
surface: sorted storage, maintenance, observability, planner-integrated vector
search, and stable GraphRAG wrappers.

Use [SQL API](docs/api.md) for the complete reference, including beta
GraphRAG helpers, legacy/manual ANN paths, and every GUC.

Use [Stability Matrix](docs/stability-matrix.md) to decide whether an API is
stable, beta, experimental, or legacy/manual before building application code
on top of it.
